"use client";

import Link from "next/link";
import { MapPin, Mail, Phone, ArrowUpRight } from "lucide-react";
import { Logo } from "@/components/logo";
import { useLanguage } from "@/context/language-context";

export function SiteFooter() {
  const { t } = useLanguage();

  const EXPLORE = [
    { label: t.nav.store, href: "/tienda" },
    { label: t.nav.about, href: "/sobre-nosotros" },
    { label: t.nav.solutions, href: "/soluciones" },
    { label: t.nav.cart, href: "/carrito" },
  ];

  const LEGAL = [
    { label: t.footer.links.terms, href: "/terminos" },
    { label: t.footer.links.privacy, href: "/privacidad" },
    { label: t.footer.links.returns, href: "/devoluciones" },
  ];

  return (
    <footer className="relative overflow-hidden bg-pine-deep text-paper">
      <div className="pointer-events-none absolute inset-0 bg-grid-dark opacity-60" />
      <div
        className="pointer-events-none absolute -right-24 -top-24 h-80 w-80 rounded-full opacity-30 blur-3xl"
        style={{ background: "radial-gradient(circle, hsl(var(--clay)), transparent 70%)" }}
      />

      <div className="container-wide relative">
        {/* CTA strip */}
        <div className="flex flex-col gap-6 border-b border-paper/10 py-12 md:flex-row md:items-end md:justify-between">
          <div className="max-w-xl">
            <p className="eyebrow text-clay-soft">{t.footer.eyebrow}</p>
            <h2 className="mt-3 font-display text-3xl leading-tight text-paper sm:text-4xl">
              {t.footer.title}
            </h2>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link
              href="/soluciones"
              className="inline-flex items-center gap-2 rounded-full bg-clay px-6 py-3.5 text-sm font-semibold text-paper transition-colors hover:bg-clay/90"
            >
              {t.footer.btnQuote}
              <ArrowUpRight className="h-4 w-4" />
            </Link>
            <Link
              href="/tienda"
              className="inline-flex items-center gap-2 rounded-full border border-paper/25 px-6 py-3.5 text-sm font-medium text-paper transition-colors hover:bg-paper/10"
            >
              {t.footer.btnCatalog}
            </Link>
          </div>
        </div>

        {/* Main */}
        <div className="grid gap-12 py-14 lg:grid-cols-12">
          <div className="lg:col-span-5">
            <Logo tone="light" />
            <p className="mt-5 max-w-sm text-sm leading-relaxed text-paper/65">
              {t.footer.desc}
            </p>

            <ul className="mt-7 space-y-4 text-sm text-paper/75">
              <li className="flex gap-3">
                <MapPin className="mt-0.5 h-5 w-5 shrink-0 text-clay-soft" />
                <span>{t.footer.address}</span>
              </li>
              <li>
                <a
                  href="mailto:atencion@curexplus.com"
                  className="flex items-center gap-3 transition-colors hover:text-paper"
                >
                  <Mail className="h-5 w-5 shrink-0 text-clay-soft" />
                  atencion@curexplus.com
                </a>
              </li>
              <li>
                <a
                  href="tel:+5215552044092"
                  className="flex items-center gap-3 transition-colors hover:text-paper"
                >
                  <Phone className="h-5 w-5 shrink-0 text-clay-soft" />
                  +52 1 55 5204 4092
                </a>
              </li>
            </ul>
          </div>

          <div className="lg:col-span-3 lg:col-start-7">
            <h3 className="font-display text-lg text-paper">{t.footer.explore}</h3>
            <ul className="mt-5 space-y-3">
              {EXPLORE.map((item) => (
                <li key={item.label}>
                  <Link
                    href={item.href}
                    className="text-sm text-paper/65 transition-colors hover:text-clay-soft"
                  >
                    {item.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          <div className="lg:col-span-3">
            <h3 className="font-display text-lg text-paper">{t.footer.legal}</h3>
            <ul className="mt-5 space-y-3">
              {LEGAL.map((item) => (
                <li key={item.label}>
                  <Link
                    href={item.href}
                    className="text-sm text-paper/65 transition-colors hover:text-clay-soft"
                  >
                    {item.label}
                  </Link>
                </li>
              ))}
            </ul>

            <h3 className="mt-8 font-display text-lg text-paper">
              {t.footer.paymentMethods}
            </h3>
            {/* Bottom Bar: Logos de Pago */}
            <div className="flex gap-2">
              <div className="px-3 py-1.5 bg-white rounded flex items-center justify-center">
                <svg className="h-4" viewBox="0 0 780 500" fill="none"><rect width="780" height="500" rx="40" fill="white"/><path fill="#1434CB" d="M293.2 348.7l33.3-190.4h53.3l-33.3 190.4h-53.3zM500.8 163c-10.5-3.9-27-8.1-47.6-8.1-52.4 0-89.3 26.4-89.6 64.2-.3 28 26.5 43.6 46.7 52.9 20.7 9.5 27.7 15.6 27.6 24.1-.1 13-16.6 19-31.9 19-21.3 0-32.6-3-50.1-10.3l-6.9-3.1-7.5 43.8c12.4 5.4 35.5 10.1 59.4 10.4 55.7 0 91.9-26.1 92.3-66.5.2-22.2-14-39.1-44.6-53-18.6-9-30-15-29.9-24.1 0-8.1 9.6-16.7 30.5-16.7 17.4-.3 30 3.5 39.8 7.5l4.8 2.3 7.2-42.4h.8zM581.8 158.3h-41c-12.7 0-22.2 3.5-27.8 16.2l-78.8 178.2h55.7l11.1-29.1h68.1l6.5 29.1H624l-42.2-194.4zm-65.6 125.2c4.4-11.2 21.3-54.4 21.3-54.4-.3.5 4.4-11.4 7.1-18.7l3.6 16.9s10.2 46.6 12.4 56.2h-44.4z"/><path fill="#1434CB" d="M239.5 158.3L187.4 289l-5.5-26.8c-9.6-30.7-39.5-64-73-80.6l47.5 166.9h56l83.2-190.2h-56.1z"/><path fill="#F7B600" d="M146.9 158.3H61.3l-.6 3.5c66.4 16 110.3 54.7 128.5 101.2l-18.5-88.8c-3.2-12.1-12.5-15.5-23.8-15.9z"/></svg>
              </div>
              <div className="px-3 py-1.5 bg-white rounded flex items-center justify-center">
                <svg className="h-4" viewBox="0 0 152 100" fill="none"><rect width="152" height="100" rx="8" fill="white"/><circle cx="55" cy="50" r="30" fill="#EB001B"/><circle cx="97" cy="50" r="30" fill="#F79E1B"/><path d="M76 27.5C82.6 32.8 87 40.8 87 50C87 59.2 82.6 67.2 76 72.5C69.4 67.2 65 59.2 65 50C65 40.8 69.4 32.8 76 27.5Z" fill="#FF5F00"/></svg>
              </div>
            </div>
          </div>
        </div>

        {/* Bottom */}
        <div className="flex flex-col items-center justify-between gap-3 border-t border-paper/10 py-7 text-xs text-paper/55 sm:flex-row">
          <p>{t.footer.copyright}</p>
          <p>{t.footer.tagline}</p>
        </div>
      </div>
    </footer>
  );
}